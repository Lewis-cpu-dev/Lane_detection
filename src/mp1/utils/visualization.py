...
def visualize_listed_prediction(list_index,images, binary_logits, instance_embeddings, binary_labels, instance_labels):
    """
    Visualize and return the combined visualization of the first image in the batch.
    """
    combined_rows = []
    for i in list_index:
	    # Input image
	    image = images[i].cpu().permute(1, 2, 0).squeeze().numpy()/255.  # Convert image to numpy

	    # Binary ground truth and prediction
	    binary_pred = torch.argmax(binary_logits[i], dim=0).cpu().numpy()  # Predicted segmentation
	    binary_gt = binary_labels[i].cpu().numpy()  # Ground truth segmentation

	    # Instance embedding visualization
	    instance_emb = instance_embeddings[i].cpu()  # Instance embedding tensor
	    instance_vis = instance_emb.squeeze()[0:3, :, :].argmax(axis=0).cpu().numpy()  # Argmax over first 3 channels
